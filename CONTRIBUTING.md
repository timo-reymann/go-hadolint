...
5. Make sure your code and commit lints (`go vet ./...`, `gofmt -s -w .`).
6. Issue that pull request!

## Bumping the bundled hadolint version

When updating the embedded hadolint binary:

1. Pick the desired upstream tag from <https://github.com/hadolint/hadolint/releases>.
2. Re-run `make fetch-binaries HADOLINT_VERSION=<tag>` to refresh the binaries, the LICENSE, and the ThirdPartyNotices file under `bin/`.
3. Run `make test` locally and in the Docker linux container.
4. Commit the binary changes alongside the version bump in `Makefile`.

## Any contributions you make will be under GPL-3.0
In short, when you submit code changes, your submissions are understood to be under the same [GPL-3.0](https://github.com/timo-reymann/go-hadolint/blob/main/LICENSE) license that covers the project. Feel free to contact the maintainers if that's a concern.

This package embeds the GPL-3.0–licensed hadolint binary, so the project as a whole is distributed under GPL-3.0.
...
- The output of `go version`, your `GOOS`/`GOARCH`, and whether `hadolint` is available on your `$PATH`
